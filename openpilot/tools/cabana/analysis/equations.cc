...
std::vector<Sample> evaluateEquation(const Equation &equation, const FieldsSnapshot &data, const std::atomic<bool> *cancelled) {
  ...
  for (const auto &sample : *source->second) {
    if (cancelled && cancelled->load(std::memory_order_relaxed)) throw std::runtime_error("Evaluation cancelled");
    setArg(0, sample.x);
    setArg(1, sample.y);
