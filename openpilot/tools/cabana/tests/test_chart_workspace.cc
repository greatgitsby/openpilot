...
// Layout-only legacy workspaces retained autogenerated route names as labels.
{
  const std::string route_a = "0123456789abcdef|2024-01-02--03-04-05";
  const std::string route_b = "fedcba9876543210|2024-02-03--04-05-06";
  REQUIRE(cabana::savedSourceRoute(Json::object{{"label", route_a}}) == route_a);
  const std::string numbered_route = "5beb9b58bd12b691|0000010a--a51155e496";
  REQUIRE(cabana::savedSourceRoute(Json::object{{"label", numbered_route}}) == numbered_route);
  REQUIRE(cabana::savedSourceRoute(Json::object{{"label", "0123456789abcdef/2024-01-02--03-04-05/2:5"}}) == route_a);
  REQUIRE(cabana::savedSourceRoute(Json::object{{"route", route_b}, {"label", route_a}}) == route_b);
  REQUIRE(cabana::savedSourceRoute(Json::object{{"route", route_a + "--7"}, {"label", "Renamed comparison"}}) == route_a);
  for (const auto &label : {"Source 1", "Same display name", "2024-01-02--03-04-05", "Live Streaming From 127.0.0.1", ""}) {
    REQUIRE(cabana::savedSourceRoute(Json::object{{"label", label}}).empty());
  }
  REQUIRE(cabana::savedSourceRoute(Json::object{}).empty());
}
{
