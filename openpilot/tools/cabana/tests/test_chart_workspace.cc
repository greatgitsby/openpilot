...
{
  const std::string route = "0123456789abcdef|2024-01-02--03-04-05";
  REQUIRE(cabana::savedSourceRoute(Json::object{{"route", route + "--7"}, {"label", "Renamed comparison"}}) == route);
  REQUIRE(cabana::savedSourceRoute(Json::object{{"label", route}}).empty());
}
{
...
// Shared source IDs may collide with routes already open in the receiving app.
{
  ...
  // User text that happens to equal an id is not a reference.
  auto labeled = document.object_items();
  labeled["name"] = "source1";
  REQUIRE(cabana::remapWorkspaceSource(labeled, "source1", "source3")["name"] == "source1");
  for (const auto &[key, value] : std::vector<std::pair<std::string, Json>>{
         {"name", ""}, {"cabana_workspace", 1}, {"widgets", Json::array{Json::object{{"kind", "can"}, {"source", "missing"}}}},
         {"sources", Json::array{Json::object{{"id", "a"}}, Json::object{{"id", "a"}}}}}) {
    auto bad = document.object_items();
    bad[key] = value;
