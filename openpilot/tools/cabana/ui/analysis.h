...
private:
  ...
  std::map<std::string, cabana::analysis::Channel> formulaInputs(const std::vector<cabana::analysis::Formula> &formulas) const;
  void refreshCan();
  void pollCan();
  void checkpoint();
  ...
  mutable std::map<std::string, cabana::analysis::Channel> can_channels_;
  std::map<std::string, cabana::analysis::Channel> custom_channels_;
  struct CanSource { MessageId id; const cabana::Signal *signal; bool sent; };
  std::map<std::string, CanSource> can_sources_;
  mutable std::set<std::string> decoded_can_;
  struct CanJob { uint64_t revision; std::future<cabana::analysis::Channel> result; };
  mutable std::map<std::string, CanJob> can_jobs_;
  mutable std::unordered_map<MessageId, std::shared_ptr<const std::vector<const CanEvent *>>> can_event_snapshots_;
  uint64_t can_result_revision_ = 0;
  std::vector<std::pair<std::string, bool>> browser_rows_;
  std::string browser_filter_;
  size_t browser_channel_count_ = 0;
  uint64_t browser_can_revision_ = 0;
  bool browser_dirty_ = true, browser_tree_ = true, browser_deprecated_ = false;
