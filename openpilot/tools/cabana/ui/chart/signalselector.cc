...
bool SignalSelector::draw() {
  ...
  if (!ImGui::BeginPopupModal(popup_id.c_str(), nullptr, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoCollapse)) {
    open_ = false;
    return false;
  }
  ...
  if (can->fields.size() != field_count_) { field_count_ = can->fields.size(); available_dirty_ = true; }
