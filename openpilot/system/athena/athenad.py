...
def startStream(sdp: str, enabled: bool, can: bool = False) -> dict:
  ...
  # CAN streams (Cabana) may also start onroad
  if params.get_bool("IsOffroad") or can:
  ...
  services_out = ["can"] if can else ["carState", "deviceState"]
  return post_stream_request(StreamRequestBody(sdp, ["wideRoad"], enabled, bridge_services_in, services_out))
