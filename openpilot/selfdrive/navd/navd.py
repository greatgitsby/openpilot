...
SPEED_LIMIT_SIGNS = {
  'mutcd': log.NavInstruction.SpeedLimitSign.mutcd,
  'vienna': log.NavInstruction.SpeedLimitSign.vienna,
}
...
class RouteEngine:
  ...
  def mapbox_token(self) -> str | None:
    return self.params.get("MapboxToken") or None
  ...
  def update_location(self):
    ...
    self.gps_ok = (self.sm.alive[self.gps_location_service] and location.hasFix and
                   location.horizontalAccuracy < GPS_ACCURACY_THRESHOLD)
    self.localizer_valid = self.gps_ok

    if not (self.gps_ok and self.sm.updated[self.gps_location_service]):
      return

    self.last_position = Coordinate(location.latitude, location.longitude)
    if location.speed > BEARING_SPEED_THRESHOLD:
      self.last_bearing = location.bearingDeg

    now = time.monotonic()
    if now - self.last_position_write > LAST_POSITION_WRITE_INTERVAL:
      self.last_position_write = now
      self.params.put("LastGPSPosition", json.dumps(self.last_position.as_dict()))
  ...
  def calculate_route(self, destination):
    ...
    waypoint_coords = self.params.get('NavDestinationWaypoints') or []
    ...
    url = MAPBOX_HOST + '/directions/v5/mapbox/driving-traffic/' + coords_str
    try:
  ...
  def send_instruction(self):
    ...
    if step.get('duration_typical') is None:
    ...
    for i in range(self.step_idx + 1, len(self.route)):
      ...
      if self.route[i].get('duration_typical') is None:
    ...
    if step.get('speedLimitSign') in SPEED_LIMIT_SIGNS:
      msg.navInstruction.speedLimitSign = SPEED_LIMIT_SIGNS[step['speedLimitSign']]
...
def main():
  ...
  sm = messaging.SubMaster([gps_location_service, 'managerState'])
