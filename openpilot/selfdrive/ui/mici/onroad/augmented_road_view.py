...
class AugmentedRoadView(CameraView):
  ...
  def _render(self, _):
    ...
    nav_active = self._nav_renderer.active

    if nav_active:
      # nav-only screen replaces the camera view; alerts render on top
      rl.draw_rectangle_rec(self._content_rect, rl.BLACK)
      self._nav_renderer.set_alert_showing(alert_to_render is not None)
    ...
    if nav_active:
      # ball fades against the black background while an alert shows
      fade = 1.0 - self._nav_renderer.ball_alpha
      if fade > 0.01:
        rl.draw_rectangle(int(self.rect.x + self.rect.width - SIDE_PANEL_WIDTH), int(self.rect.y),
                          SIDE_PANEL_WIDTH, int(self.rect.height), rl.Color(0, 0, 0, int(255 * fade)))
