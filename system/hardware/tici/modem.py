...
class Modem:
  ...
  def _do_searching(self):
    ...
    if reg == "roaming" and not self._roaming_allowed:
      self._publish_state(registration=reg, error={"type": "roaming_disabled", "description": "Roaming is disabled."})
      return State.SEARCHING

    if reg in ("home", "roaming") and greg in ("home", "roaming"):
      self._publish_state(registration=reg, error={})
      return State.CONNECTING

    if reg != self.S.get("registration"):
      self._publish_state(registration=reg, error=self._carrier_reject_error(reg))
    return self._searching_idle()
