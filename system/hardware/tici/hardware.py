...
class Tici(HardwareBase):
  ...
  def _modem_state(self) -> dict:
    """Read modem state from /dev/shm/modem (written by modem.py)."""
    try:
  ...
  def get_network_type(self):
    ...
    # check modem state file for cellular
    ms = self._modem_state()
    if ms.get("connected"):
      nt = ms.get("network_type", "")
      if nt == "lte":
        return NetworkType.cell4G
      elif nt in ("utran", "utran_hsdpa", "utran_hsupa", "utran_hsdpa_hsupa"):
        return NetworkType.cell3G
      elif nt in ("gsm", "gsm_egprs"):
  ...
  def get_imei(self, slot):
    if slot != 0:
      return ""
    return self._modem_state().get("imei", "")
  ...
  def get_network_info(self):
    ...
    if not ms.get("band"):
      return None
    return {
      'technology': ms.get("network_type", ""),
      'operator': ms.get("operator", ""),
      'band': ms.get("band", ""),
      'channel': min(ms.get("channel", 0), 65535),  # capnp UInt16
      'extra': ms.get("extra", ""),
      'state': ms.get("state", ""),
    }
  ...
  def get_modem_version(self):
    return self._modem_state().get("modem_version") or None

  def get_modem_temperatures(self):
    return self._modem_state().get("temperatures", [])
  ...
  def configure_modem(self):
    # AT init commands are now handled by modem.py on boot
    pass

  def reboot_modem(self):
    # modem.py handles recovery via lte.sh escalation
    os.system("sudo systemctl restart modem.service 2>/dev/null")
  ...
  def get_modem_data_usage(self):
    ms = self._modem_state()
    tx = ms.get("tx_bytes", -1)
    rx = ms.get("rx_bytes", -1)
