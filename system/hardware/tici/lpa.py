...
def _cancel_session_safe(client: AtClient, smdp: str, tx_id: str, tx_id_bytes: bytes, session: requests.Session) -> None:
  b64_cancel = ""
  try:
    b64_cancel = cancel_session(client, tx_id_bytes)
  except Exception:
    pass
  try:
...
def download_profile(client: AtClient, activation_code: str) -> str:
  ...
  try:
    ...
    _cancel_session_safe(client, smdp, tx_id, tx_id_bytes, session)
    raise
